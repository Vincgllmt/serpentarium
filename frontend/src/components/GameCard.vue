...
import { partLabel } from '../utils/grouping'

const { group } = defineProps<{ group: GameGroup }>()
const emit = defineEmits<{ updated: [game: Game] }>()

const game = computed(() => group.primary)
const downloadUrl = computed(() => `${API_URL}/api/games/${game.value.id}/download`)
...
const coverUrl = computed(() =>
  game.value.cover_url?.startsWith('/') ? `${API_URL}${game.value.cover_url}` : game.value.cover_url,
)
...
function variantDownloadUrl(variantId: number) {
  return `${API_URL}/api/games/${variantId}/download`
}
...
async function rescrape() {
  ...
  try {
    const res = await fetch(`${API_URL}/api/games/${game.value.id}/enrich?force=true`, { method: 'POST' })
  ...
  <div>
    ...
    >
      ...
      <button
        ...
        type="button"
        :title="`${group.variants.length} fichiers`"
        class="absolute left-2 top-2 flex items-center gap-1 rounded-full bg-black/60 px-2 py-1 text-xs font-semibold text-white ring-1 ring-white/20 hover:bg-black/80"
    ...
    <div v-if="expanded" class="mt-2 space-y-1 rounded-lg bg-white/5 p-2 ring-1 ring-white/10">
      <a
        v-for="variant in group.variants"
        :key="variant.id"
        :href="variantDownloadUrl(variant.id)"
        class="flex items-center justify-between gap-2 rounded px-1.5 py-1 text-xs text-white/80 hover:bg-white/10 hover:text-white"
      >
        <span class="truncate">{{ partLabel(variant, group) }}</span>
        <span class="whitespace-nowrap text-white/40">{{ formatBytes(variant.size) }}</span>
      </a>
    </div>
  </div>
</template>
